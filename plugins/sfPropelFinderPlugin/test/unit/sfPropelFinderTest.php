...
$finder = sfPropelFinder::from('Article')->relatedTo($category1);
$articles = $finder->find();
$t->is(count($articles), 2, 'relatedTo() filters results to the ones relative to a record');
foreach($articles as $article)
{
  $t->is($article->getCategoryId(), $category1->getId(), 'relatedTo() filters results to the ones relative to a record');
}
$finder = sfPropelFinder::from('Article')->relatedTo($category2);
$articles = $finder->find();
$t->is(count($articles), 1, 'relatedTo() filters results to the ones relative to a record');
foreach($articles as $article)
{
  $t->is($article->getCategoryId(), $category2->getId(), 'relatedTo() filters results to the ones relative to a record');
}
...
$finder = sfPropelFinder::from('Article')->
  join('Comment')->
  withColumn('Article.Id', 'foo')->
  groupBy('foo')->
  withColumn('COUNT(comment.ID)', 'NbComments');
$article = $finder->findOne();
$t->is($finder->getLatestQuery(), 'SELECT article.ID, article.TITLE, article.CATEGORY_ID, article.ID AS foo, COUNT(comment.ID) AS NbComments FROM article INNER JOIN comment ON (article.ID=comment.ARTICLE_ID) GROUP BY foo LIMIT 1', 'groupBy() accepts a column alias and issues a GROUP BY clause');
...
$finder = sfPropelFinder::from('Article')->
  ...
  withColumn('COUNT(comment.ID)', 'NbComments');
$article = $finder->findOne();
$t->is($finder->getLatestQuery(), 'SELECT article.ID, article.TITLE, article.CATEGORY_ID, COUNT(comment.ID) AS NbComments FROM article INNER JOIN comment ON (article.ID=comment.ARTICLE_ID) GROUP BY article.ID LIMIT 1', 'groupByXXX() accepts a column name and issues a GROUP BY clause');
...
$t->is($finder->getLatestQuery(), 'UPDATE article SET TITLE = \'updated title\' WHERE article.ID|1', 'set() issues an update query even when passed an empty finder');
...
$t->is(sfPropelFinder::from('Article')->where('Title', 'updated title')->count(), 2, 'set() updates only the records found based on the array of values');
...
$finder = sfPropelFinder::from('Article')->where('Title', 'foo');
$t->isa_ok($finder->getCriteria(), 'Criteria', 'getCriteria() returns the criteria as composed by the finder');
$finder->findOne();
